...
from nonebot.adapters.kaiheila.event import MessageEvent as KookMEvent
...
def get_user_id():
    def dependency(
        bot: Union[V11Bot, V12Bot, KookBot],
        event: Union[V11MEvent, V12MEvent, KookMEvent],
    ) -> str:
        if isinstance(event, (V11MEvent, KookMEvent)):
            cid = f"{bot.self_id}_{event.message_type}_"
        else:
            cid = f"{bot.self_id}_{event.detail_type}_"

        if isinstance(event, (V11GMEvent, V12GMEvent)):
...
async def _(
    ...
    if not meme_list_cache_file.exists():
        ...
        with meme_list_cache_file.open("wb") as f:
    ...
    await MessageFactory([Image(img), Text(msg)]).finish()
...
async def _(
    bot: Union[V11Bot, V12Bot, KookBot],
    matcher: Matcher,
    msg: Union[V11Msg, V12Msg, KookMsg] = CommandArg(),
):
    ...
    await MessageFactory([Image(img), Text(info)]).finish()
    # if isinstance(bot, V11Bot):
...
@block_cmd_gl.handle()
async def _(matcher: Matcher, msg: Union[V11Msg, V12Msg, KookMsg] = CommandArg()):
    meme_names = msg.extract_plain_text().strip().split()
    if not meme_names:
        matcher.block = False
        await matcher.finish()
...
async def process(
    ...
    await MessageFactory([Image(result)]).finish()
    # if isinstance(bot, V11Bot):
...
async def create_matchers():
    for meme in meme_manager.memes:
        ...
        for matcher in matchers:
            ...
            matcher.append_handler(
                handler(meme),
                parameterless=[await split_msg_kook(meme)],
            )
    ...
    random_matcher.append_handler(
        random_handler,
        parameterless=[split_msg_v11(fake_meme)],
    )
    random_matcher.append_handler(
        random_handler,
        parameterless=[split_msg_v12(fake_meme)],
    )


asyncio.run(create_matchers())
