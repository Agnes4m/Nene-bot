...
class KookAvatar(ImageSource):
    ...
    async def get_image(self) -> bytes:
        user_info = await get_user_info(self.bot, self.event, self.user_id)
        if user_info and user_info.user_avatar:
            avatar = await user_info.user_avatar.get_image()
        else:
            avatar = await download_url("https://nonebot.dev/logo.png")
        return avatar


def user_avatar(
    bot: Union[V11Bot, V12Bot, KookBot],
    event: Union[V11MEvent, V12MEvent, KookMsgEvent],
    user_id: str,
) -> ImageSource:
    if isinstance(bot, V11Bot):
        return QQAvatar(qq=user_id)
    if isinstance(bot, KookBot) and isinstance(event, KookMsgEvent):
        return KookAvatar(bot, event, user_id)
    assert isinstance(event, V12MEvent) and isinstance(bot, V12Bot)
