...
async def get_shindan_title(id_: str) -> str:
    url = f"https://shindanmaker.com/{id_}"
    async with httpx.AsyncClient() as client:
        resp = await get(client, url)
...
async def make_shindan(id_: str, name: str, mode="image") -> Union[str, bytes]:
    url = f"https://shindanmaker.com/{id_}"
...
def remove_shindan_effects(content: Tag, type_: str):
    for tag in content.find_all("span", {"class": "shindanEffects", "data-mode": type_}):
