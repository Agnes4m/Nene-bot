...
from nene.utils_.event import M
...
async def get_sender_username(bot: Bot, event: Event):
    user_info = await get_user_info(bot, event, event.get_user_id())
    if user_info:
        return user_info.user_remark or user_info.user_displayname or user_info.user_name
    return None
