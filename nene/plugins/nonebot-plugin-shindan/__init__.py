...
async def _(matcher: Matcher, msg: Message = CommandArg()):
    ...
    id_ = args[0]
    command = args[1]
    title = await get_shindan_title(id_)
    if not title:
        await matcher.finish("找不到该占卜，请检查id")

    if resp := await shindan_manager.add_shindan(id_, command, title):
...
async def _(matcher: Matcher, msg: Message = CommandArg()):
    ...
    if resp := await shindan_manager.remove_shindan(id_):
        await matcher.finish(resp)
    else:
...
async def _(matcher: Matcher, msg: Message = CommandArg()):
    ...
    if resp := await shindan_manager.set_shindan_mode(id_, mode):
        await matcher.finish(resp)
    else:
...
async def _(state: T_State, matcher: Matcher):
    ...
    try:
        res = await make_shindan(id_, name, mode)
    except Exception:
