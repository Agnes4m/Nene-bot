...
utils = on_command("txt2img", rule=to_me())


@utils.got("TITLE", prompt="请输入标题（空格表示留空）")
@utils.got("TEXT", prompt="请输入内容")
@utils.got("SIZE", prompt="请输入字体大小")
async def txt2img_handle(
    ...
    if size.isdigit():
        if 20 <= int(size) <= 120:
            ...
            await utils.reject("字体大小需要在20到120之间，请重新输入")
    else:
        await utils.reject("字体大小格式有误，请输入20到120之间的数字")
